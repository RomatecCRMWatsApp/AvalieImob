...
**Estado atual: v1.3.727** (MAJOR=1, MINOR=3) — release: Propostas — +2 engines portados e VERIFICADOS contra a ZAYRA: Retificação de Área (services/pricing/retificacao.py, admin/judicial) e Avaliação PTAM (ptam.py, faixas SM + fatores precisão/finalidade). Habilitados no dispatcher + catálogo (disponivel=True). PropostaForm.jsx refatorado para schema-driven (SCHEMAS por subtipo → campos), agora cobre averbação resid/comercial + retificação + PTAM com preview ao vivo. 4 de 11 tipos disponíveis. PENDENTE: georref, demarcação urbana/rural, desmembramento, remembramento, projeto executivo, desmembramento de obra; adicional de campo; PDF. Release anterior abaixo:
