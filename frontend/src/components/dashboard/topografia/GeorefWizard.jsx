...
// Exige PNG com fundo TRANSPARENTE (senão ganharia caixa branca sobre o tema verde).
function LogoBranding({ toast }) {
  ...
  const validarPng = (file) => new Promise((resolve) => {
    if (!/png/i.test(file.type) && !/\.png$/i.test(file.name)) {
      resolve('O logo precisa ser um arquivo PNG.'); return;
    }
    const img = new Image();
    img.onload = () => {
      try {
  ...
  const onPick = async (file) => {
    ...
    try {
      ...
      await carregar();
      toast({ title: 'Logo aplicado ✓', description: 'Vai sair no cabeçalho de todas as peças geradas.' });
    } catch (e) {
  ...
  return (
    <div className="mt-4 border rounded-lg p-3 bg-gray-50">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        {temCustom && (
          <button onClick={remover} disabled={busy} className="text-xs text-gray-400 hover:text-red-500">Remover</button>
        )}
      </div>
    </div>
  );
