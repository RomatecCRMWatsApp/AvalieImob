...
import {
  FileText, Users, Building2, DollarSign, TrendingUp, Clock,
  Loader2, Plus, BarChart2, CreditCard, Calendar, CheckCircle,
  AlertCircle, Zap, ChevronRight, ArrowUpRight
} from 'lucide-react';
...
/* ─── Stat card ─────────────────────────────────────────── */
const StatCard = ({ icon: Icon, label, value, iconBg, iconColor, delta, href }) => (
  <Link
    to={href || '#'}
...
const Shortcut = ({ icon: Icon, label, to, accent }) => (
  <Link
    to={to}
    className={`flex items-center gap-3 px-4 py-3 rounded-xl border text-sm font-semibold
                ...
                ${accent
                  ...
                  : 'bg-white border-gray-200 text-gray-700 hover:border-emerald-200 hover:text-emerald-800'
                }`}
...
/* ─── Bar chart column ──────────────────────────────────── */
const BarCol = ({ m, pct, isMax }) => (
  <div className="flex-1 flex flex-col items-center gap-2 group">
    <div className="text-[10px] font-semibold text-gray-500 group-hover:text-gray-800 transition-colors">
      {m.count}
    </div>
...
const DashOverview = () => {
  ...
  const planObj   = PLANS.find(p =>
    p.name.toLowerCase() === planLabel?.toLowerCase() ||
    p.id.toLowerCase()   === planLabel?.toLowerCase()
  );
  const planExpires = payStatus?.plan_expires
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex items-center gap-2 bg-white border border-gray-200 rounded-xl px-4 py-2.5 shadow-sm self-start sm:self-auto">
          <div
            ...
            style={{ background: planSt === 'active' ? '#16a34a' : '#6b7280' }}
          />
          <span className="text-xs font-semibold text-gray-700 capitalize">{planLabel}</span>
          <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${subInfo.bg} ${subInfo.text}`}>
            {subInfo.label}
          </span>
          {planExpires && (
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          icon={FileText}  label="Laudos emitidos"
          value={stats.evaluations}
          iconBg="bg-emerald-100" iconColor="text-emerald-800"
          delta="+12%"
          href="/dashboard/ptam"
        />
        <StatCard
        ...
        />
        <StatCard
          icon={DollarSign}  label="Volume avaliado"
          value={`R$ ${(stats.revenue).toLocaleString('pt-BR', { maximumFractionDigits: 0 })}`}
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex items-center gap-3 text-[10px] font-semibold text-gray-500">
              ...
              <span className="flex items-center gap-1.5">
                <span className="w-3 h-3 rounded-sm inline-block" style={{ background: DARK_GREEN }} />
                Pico
              </span>
            </div>
          </div>
                ...
                {recent.map((e, i) => {
                  ...
                  return (
                    <div key={e.id} className="flex gap-3 relative">
                      {/* Timeline dot */}
                      <div
                        className="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 border-2 border-white shadow-sm z-10"
                        style={{ background: i === 0 ? GOLD : '#f3f4f6' }}
                      >
                      ...
                      <div className="flex-1 min-w-0 pb-1">
                        <div className="flex items-center justify-between gap-2 flex-wrap">
                          ...
                          <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full whitespace-nowrap ${statusCls}`}>
                            {e.status}
                          </span>
                        </div>
      ...
      <div
        ...
        style={{ background: `linear-gradient(135deg, ${DARK_GREEN} 0%, #2a7a2a 100%)` }}
      >
          ...
          <div>
            ...
            {planObj && (
              <div className="text-xs text-emerald-200 mt-0.5">
                {planObj.features[0]} · {planObj.features[1]}
              </div>
            )}
          </div>
        </div>
        <div className="flex items-center gap-3 flex-wrap">
          <span className={`text-xs font-bold px-3 py-1.5 rounded-full flex items-center gap-1.5 ${subInfo.bg} ${subInfo.text}`}>
            <SubIcon className="w-3.5 h-3.5" />
            {subInfo.label}
          </span>
          {planExpires && (
            <span className="text-xs text-emerald-200 flex items-center gap-1">
              <Calendar className="w-3.5 h-3.5" /> Expira {planExpires}
            </span>
          )}
          <Link
