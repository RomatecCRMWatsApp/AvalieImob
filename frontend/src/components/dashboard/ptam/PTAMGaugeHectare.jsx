// @module ptam/PTAMGaugeHectare — Gauge âmbar 72px com valor por hectare (PTAM rural).
// Preenche o arco proporcional ao VTI médio da tabela INCRA (se houver); senão 75% visual.
import React from 'react';

const CIRCUNFERENCIA = 188.4;
  ...
  return (
    <div style={{ position: 'relative', width: 72, height: 72, flexShrink: 0 }}>
      <svg width="72" height="72" viewBox="0 0 72 72" style={{ position: 'absolute', top: 0, left: 0 }}>
        <circle cx="36" cy="36" r="30" fill="none" stroke="#1a1505" strokeWidth="5" />
        <circle
          cx="36" cy="36" r="30" fill="none" stroke="#f59e0b" strokeWidth="5"
      ...
      <div style={{
        ...
        <span style={{ fontSize: 7, color: '#94a3b8', textTransform: 'uppercase', lineHeight: 1.2 }}>
          Valor por<br />hectare
        </span>
        <span style={{ fontSize: 9, fontWeight: 600, color: '#f59e0b', margin: '2px 0', lineHeight: 1.2 }}>
          {fmtBRL0(valorHa)}
        </span>
        <span style={{ fontSize: 7, color: '#64748b' }}>/ ha</span>
      </div>
    </div>
  );
}
