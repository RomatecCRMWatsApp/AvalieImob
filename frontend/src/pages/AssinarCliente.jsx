...
export default function AssinarCliente() {
  ...
  const setupCanvas = useCallback(() => {
    ...
    ctx.setTransform(dpr, 0, 0, dpr, 0, 0); // desenhar em CSS px
  ...
  return (
    <div style={page}>
      ...
      <div style={inner}>
        ...
        <p style={{ margin: '2px 0', fontSize: 14 }}>Você assina como <b style={{ color: DOURADO }}>{ROLE_LABEL[info?.role] || info?.role}</b>.</p>
          ...
          <div style={{ marginTop: 12 }}>
            ...
            {info.documentos.map((d) => (
              <div key={d.tipo} style={{ marginBottom: 12 }}>
                <div style={{ fontWeight: 600, color: '#fff', margin: '4px 0', fontSize: 14 }}>{d.titulo}</div>
                {(d.paginas || []).map((pg) => {
                    ...
                    <div key={pg.pagina} style={{ position: 'relative', marginBottom: 6, borderRadius: 8, overflow: 'hidden', border: '1px solid #1c3a2c' }}>
                      ...
                      {boxes.map((b, i) => (
                        <div key={i} style={{
                          ...
                          <span style={{ position: 'absolute', top: -20, left: 0, background: DOURADO, color: '#1a1a1a', fontSize: 10, fontWeight: 700, padding: '2px 6px', borderRadius: 5, whiteSpace: 'nowrap' }}>➜ sua assinatura aqui</span>
                        </div>
                      ))}
                    </div>
                  );
