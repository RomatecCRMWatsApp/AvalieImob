...
@api.get("/")
async def root():
    return {"app": "RomaTec AvalieImob API", "version": "1.11.1"}
...
async def sitemap():
    ...
    paginas = [
        ...
        ("/blog/contrato-exclusividade-procuracao-corretor-imoveis", "0.8", "monthly"),
        ("/blog/assinatura-icp-brasil-laudos-contratos-validade-juridica", "0.8", "monthly"),
        ("/blog/precificar-georreferenciamento-demarcacao-averbacao-proposta", "0.8", "monthly"),
        ("/contato",                     "0.7", "monthly"),
...
async def startup():
    ...
    # Assinatura externa BYOK — 1 credencial por (usuário, provedor).
    try:
        await db.assinatura_credenciais.create_index([("user_id", 1), ("provider", 1)], unique=True)
        await db.assinatura_credenciais.create_index([("user_id", 1), ("padrao", 1)])
        await db.assinatura_envios.create_index([("user_id", 1), ("created_at", -1)])
