...
class LocacaoAmostra(BaseModel):
    """Amostra de mercado para avaliação de locação."""
    address: Optional[str] = ""
    neighborhood: Optional[str] = ""
    area: Optional[float] = 0
...
class LocacaoBase(BaseModel):
    ...
    numero_locacao: Optional[str] = None   # gerado automaticamente (ex: LOC-0001/2026)
    solicitante_nome: Optional[str] = ""
    ...
    # ── Seção 2 — Objetivo ───────────────────────────────────────────────────
    objetivo: Optional[str] = ""        # fixacao | revisao | renovatoria | outros
    objetivo_outros: Optional[str] = ""
    tipo_locacao: Optional[str] = "residencial"   # residencial | comercial | galpao | ponto_comercial | misto
    ...
    imovel_tipo: Optional[str] = ""     # casa | apartamento | sala_comercial | galpao | loja | terreno
    imovel_area_terreno: Optional[float] = 0
    imovel_area_construida: Optional[float] = 0
    imovel_idade: Optional[int] = 0
    imovel_estado_conservacao: Optional[str] = ""   # otimo | bom | regular | ruim | pessimo
    ...
    # ── Seção 7 — Garantia ───────────────────────────────────────────────────
    prazo_locacao: Optional[str] = ""           # ex: '30 meses'
    garantia_locacao: Optional[str] = ""        # caucao | fiador | seguro_fianca | titulo_capitalizacao | nenhuma
    fator_locacao: Optional[float] = None       # percentual aplicado sobre valor venal (ex: 0.5%)
    ...
    # ── Seção 10 — Resultado ─────────────────────────────────────────────────
    valor_locacao_estimado: Optional[float] = None    # valor mensal estimado
    valor_locacao_minimo: Optional[float] = None      # faixa mínima mensal
    valor_locacao_maximo: Optional[float] = None      # faixa máxima mensal
